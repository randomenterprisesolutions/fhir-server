...
Old search parameters are deleted whenever a resource is updated. When remote indexing is enabled, this means that a resource will not be searchable until the remote index service has received and processed the message. Carefully examine your interaction scenarios with the randomenterprisesolutions FHIR Server to determine if this behavior is suitable. In particular, conditional updates are unlikely to work as expected because the search may not return the expected value, depending on timing.
...
| ------ | --- | ----------- |
| db.host | All | The host name of the database containing the randomenterprisesolutions FHIR Server schema | 

...
Quickstart:
```
docker run -p 9443:9443 -e BOOTSTRAP_DB=true ghcr.io/randomenterprisesolutions/fhir-server
```

Note:
1. The Docker image [ghcr.io/randomenterprisesolutions/fhir-schematool](https://github.com/randomenterprisesolutions/fhir-server/pkgs/container/fhir-schematool) is an early technology preview and is experimental.
2. The Docker image [ghcr.io/randomenterprisesolutions/fhir-bucket-tool](https://github.com/randomenterprisesolutions/fhir-server/pkgs/container/fhir-bucket-tool) is an early technology preview and is experimental.
3. The Docker image [ghcr.io/randomenterprisesolutions/fhir-term-loader](https://github.com/randomenterprisesolutions/fhir-server/pkgs/container/fhir-term-loader) is an early technology preview and is experimental.

### Building with the randomenterprisesolutions FHIR Modules
The randomenterprisesolutions FHIR Server modules are **not published to Maven Central**. To use the modules, build and install them locally:
...
Then declare the dependencies in your project. For example, to use the visitable, thread-safe FHIR object model (including high-performance parsers and generators), declare a dependency on the `fhir-model` module:
...
### randomenterprisesolutions FHIR modules
The randomenterprisesolutions FHIR Server is modular and extensible. The following tables provide an overview of all the modules, along with an indicator of the stability of the Java APIs defined in each module. This indicator is only applicable to the direct usage of the modules, not for usage of the randomenterprisesolutions FHIR Server as a whole.

#### Core
|Module|Description|Java API-stable|
|------|-----------|----------|
|fhir-parent|The parent project for all projects which make up the randomenterprisesolutions FHIR Server|false|
...
|conformance/fhir-ig-us-core|Packaging the US Core Implementation Guide for the randomenterprisesolutions FHIR registry|false|
|conformance/fhir-ig-mcode|Packaging the minimal Common Oncology Data Elements for the randomenterprisesolutions FHIR registry|false|
|conformance/fhir-ig-carin-bb|Packaging the Consumer-Directed Payer Data Exchange Guide for the randomenterprisesolutions FHIR registry|false|
|conformance/fhir-ig-davinci-pdex|Packaging the Da Vinci Payer Data Exchange (PDEX) Implementation Guide for the randomenterprisesolutions FHIR registry|false|
|conformance/fhir-ig-davinci-hrex|Packaging the Da Vinci Health Record Exchange (HREX) Implementation Guide for the randomenterprisesolutions FHIR registry|false|
|conformance/fhir-ig-davinci-pdex-plan-net|Packaging the Da Vinci Payer Data Exchange (PDEX) Plan Net Implementation Guide for the randomenterprisesolutions FHIR registry|false|
|conformance/fhir-ig-davinci-pdex-formulary|Packaging the Da Vinci Payer Data Exchange (PDex) US Drug Formulary Implementation Guide for the randomenterprisesolutions FHIR registry|false|
...
|Module|Description|Java API-stable|
|------|-----------|----------|
|fhir-client|A FHIR Client that re-uses the randomenterprisesolutions FHIR Server model and its JAX-RS Providers|false|
...
|fhir-benchmark|Java Microbenchmark Harness (JMH) tests for measuring read/write/validation performance for the randomenterprisesolutions FHIR Server and the HL7 FHIR Java Reference Implementation|false|
|fhir-bucket|Scans cloud object storage buckets and uploads data using the FHIR REST API|false|
|fhir-persistence-schema|Classes for deploying and updating the randomenterprisesolutions FHIR Server relational database schema|false|
|fhir-persistence-cassandra-app|CLI utility application supporting payload storage to Cassandra *experimental* |false|

### Contributing to the randomenterprisesolutions FHIR Server
The randomenterprisesolutions FHIR Server is under active development. To help develop the server, clone or download the project and build it using Maven.
See [Setting up for development](https://github.com/randomenterprisesolutions/fhir-server/wiki/Setting-up-for-development) for more information.
...
### License
The randomenterprisesolutions FHIR Server and its corresponding modules are licensed under the Apache 2.0 license.
The full license text is available at [LICENSE](LICENSE).

This project is a fork of the [LinuxForHealth FHIR Server](https://github.com/LinuxForHealth/FHIR), which is itself derived from the IBM® FHIR® Server. Copyright notices for the original work are preserved in the [NOTICE](NOTICE) file, as required by the Apache 2.0 license.

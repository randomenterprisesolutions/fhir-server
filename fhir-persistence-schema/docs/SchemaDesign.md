# The randomenterprisesolutions FHIR Server - Schema Design and Management

The randomenterprisesolutions FHIR Server schema comes in two flavors:
...
The randomenterprisesolutions FHIR schema is managed in code and is applied to a target database using a JDBC connection. This simplifies development of Derby-based unit-tests, allows a common schema definition to be used across multiple database flavors and simplifies deployment in cloud scenarios.
...
The randomenterprisesolutions FHIR Server persists resources using tables defined in a data schema (usually FHIRDATA, but this can be anything). Multiple data schemas can exist in the same database, all managed from a single FHIR_ADMIN schema.
...
The following table describes the purpose of each table or group of tables in the randomenterprisesolutions FHIR Server data schema:
...
Rows in the `LOGICAL_RESOURCES` are locked for update during ingestion to protect data integrity. Only the rows for the resources being changed are locked. The randomenterprisesolutions FHIR Server tries hard to apply locks in a deterministic order to avoid deadlocks but this isn't always possible. Deadlocks may sometimes occur when processing transaction bundles involving overlapping data.
...
The randomenterprisesolutions FHIR Server uses soft-delete when processing a FHIR `DELETE` interaction. This creates a new version of the resource with a minimal resource data value and the `IS_DELETED` flag = `Y`. Soft-deletes do not delete existing database records. The `$erase` custom operation can be used to remove all traces of a resource from a database.

All timestamps stored in the randomenterprisesolutions FHIR Server schema are UTC.

For more details on how the resource payload data is stored, see the next section.

## Scanning Resources

The randomenterprisesolutions FHIR Server implements the whole-system `_history` endpoint to fetch resources in the order they were ingested into the system. This endpoint is described in the randomenterprisesolutions FHIR Server [Conformance Guide](https://randomenterprisesolutions.github.io/fhir-server/Conformance/#whole-system-history). This service is backed by the `RESOURCE_CHANGE_LOG` table which records the identity of each resource version as it is ingested. The table is indexed on `CHANGE_TSTAMP` which reflects the UTC last-modified timestamp for that version of the resource.
...
The resource object can be more easily retrieved from the randomenterprisesolutions FHIR Server REST API using a versioned read (VREAD) interaction:
...
With each release of the randomenterprisesolutions FHIR Server, these tests should be expanded to cover [at least] the migrations from the previous version.
...
For the `DISTRIBUTED` schema type variant, uniqueness of the {resource_type_id, logical_id} tuple can no longer be enforced by the above unique index on `logical_resources`, because `logical_resources` is distributed by `logical_resource_id` and this column is not part of the index definition. Instead, the randomenterprisesolutions FHIR Server relies on the new `logical_resource_ident` table to manage uniqueness of this tuple, and the `resource_type_id` and `logical_id` columns in `logical_resources` are just denormalized copies of the data.
...
- [Git Issue: Document the schema migration process on the project wiki #270](https://github.com/randomenterprisesolutions/FHIR/issues/270)

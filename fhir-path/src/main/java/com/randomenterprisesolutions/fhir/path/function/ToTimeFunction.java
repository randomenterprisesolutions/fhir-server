...
import static com.randomenterprisesolutions.fhir.path.util.FHIRPathUtil.convertsToTime;
...
import static com.randomenterprisesolutions.fhir.path.util.FHIRPathUtil.toTime;
...
public class ToTimeFunction extends FHIRPathAbstractFunction {
    @Override
    ...
    public Collection<FHIRPathNode> apply(EvaluationContext evaluationContext, Collection<FHIRPathNode> context, List<Collection<FHIRPathNode>> arguments) {
        if (!convertsToTime(context)) {
            return empty();
        }
        return singleton(toTime(context));
    }
}

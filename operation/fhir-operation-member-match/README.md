...
The strategy may be extended using a JAR that implements the service loader. To register a JAR, `META-INF/services/com.randomenterprisesolutions.fhir.operation.davinci.hrex.provider.strategy.MemberMatchStrategy` the file must point to the package and class that implements `MemberMatchStrategy`.

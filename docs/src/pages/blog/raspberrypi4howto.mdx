...
![IBM FHIR Server on Raspberry Pi 4](https://randomenterprisesolutions.github.io/fhir-server/images/piOnFhir30.jpg)
...
The default password used throughout is `change-password` and this is configured as clear-text in the configuration files to make it even more obvious that it should be changed as part of any hardening. Refer to the [FHIRServerUsersGuide](https://randomenterprisesolutions.github.io/fhir-server/guides/FHIRServerUsersGuide) for more details.
...
```
mkdir -p ~/git && cd ~/git && git clone https://github.com/randomenterprisesolutions/FHIR.git
```

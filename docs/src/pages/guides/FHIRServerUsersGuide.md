    ...
    "fhirServer":{
        ...
        "persistence":{
            ...
            "datasources": {
                "study1": {
                    "type": "postgresql",
                    "currentSchema": "study1data"
                },
                "study2": {
                    "type": "postgresql",
                    "currentSchema": "study2data"
                }
            }
        }
    }
...
<server>
    ...
    <!-- ============================================================== -->
    <dataSource id="fhirDatasourcePgAcmeStudy1" jndiName="jdbc/fhir_acme_study1" type="javax.sql.XADataSource" statementCacheSize="200" syncQueryTimeoutWithTransactionTimeout="true" validationTimeout="30s">
        <jdbcDriver javax.sql.DataSource="org.postgresql.xa.PGDataSource" libraryRef="sharedLibPostgres"/>
        <properties.postgresql
             serverName="dbserver"
             portNumber="5432"
             databaseName="fhirdb"
    ...
    <!-- ============================================================== -->
    <dataSource id="fhirDatasourcePgAcmeStudy2" jndiName="jdbc/fhir_acme_study2" type="javax.sql.XADataSource" statementCacheSize="200" syncQueryTimeoutWithTransactionTimeout="true" validationTimeout="30s">
        <jdbcDriver javax.sql.DataSource="org.postgresql.xa.PGDataSource" libraryRef="sharedLibPostgres"/>
        <properties.postgresql
             serverName="dbserver"
             portNumber="5432"
...
```
{
    ...
    "fhirServer": {
        ...
        "persistence": {
            "factoryClassname": "com.randomenterprisesolutions.fhir.persistence.blob.FHIRPersistenceJDBCBlobFactory",
            "datasources": {
                "default": {
                    ...
                }
...
1. Implement each operation as a Java class that extends `com.randomenterprisesolutions.fhir.operation.AbstractOperation` from `fhir-operation.jar`. Ensure that your implementation returns an appropriate `OperationDefinition` in its `getDefinition()` method, because the framework validates both the request and response payloads to ensure that they conform to the definition.

2. Create a file named `com.randomenterprisesolutions.fhir.operation.FHIROperation` with one or more fully qualified `FHIROperation` classnames and package it in your jar under `META-INF/services/`.
...
2.  Store the fully-qualified classname of your interceptor implementation class in a file called :
      ...
      `META-INF/services/com.randomenterprisesolutions.fhir.server.spi.interceptor.FHIRPersistenceInterceptor`
...
``` json
{
    ...
    "fhirServer":{
        ...
        "audit": {
            "serviceClassName" : "com.randomenterprisesolutions.fhir.audit.impl.NopService",
            "serviceProperties" : {
            }
        },
        "persistence":{
...
```
"audit": {
    "serviceClassName" : "com.randomenterprisesolutions.fhir.audit.impl.KafkaService",
    "serviceProperties" : {
        "load": "config",
    }
```
...
|`fhirServer/audit/serviceClassName`|string|The audit service to use. Currently, com.randomenterprisesolutions.fhir.audit.impl.NopService to indicate the logger service is disabled, and com.randomenterprisesolutions.fhir.audit.impl.KafkaService to indicate using Kafka as a destination.|
...
|`fhirServer/persistence/factoryClassname`|com.randomenterprisesolutions.fhir.persistence.jdbc.FHIRPersistenceJDBCFactory|
...
- <b id="f10">9</b> These instructions assume the use of a basic user registry in the server.xml file.  If you are instead using an LDAP registry, then the entire DN associated with the client certificate must match the DN of a user in the LDAP registry. [↩](#a10)

...
The [FHIR Specification](https://hl7.org/fhir/R4B/search.html) defines a set of searchable fields for each resource type. The randomenterprisesolutions FHIR Server supports searching via both specification-defined search parameters and tenant-specific search parameters.

Specifically, the randomenterprisesolutions FHIR Server supports searching on additional fields, including:
...
The randomenterprisesolutions FHIR Server allows deployers to define search parameters on a tenant-specific basis. This allows each tenant to share an instance of the randomenterprisesolutions FHIR Server while maintaining the ability to have their own set of search parameters. Additionally, specification-defined search parameters can be filtered out in order to avoid the cost of extracting and storing the corresponding indices.
...
The randomenterprisesolutions FHIR Server contains a FHIRRegistryResourceProvider that reads `extension-search-parameters.json` files from the tenant config directory and contributes their contents to the registry.

The randomenterprisesolutions FHIR Server also supports compartment search based on CompartmentDefinition resources that are loaded from the registry.
...
```json
{
  ...
  "resource": {
    ...
    "publisher": "randomenterprisesolutions FHIR Server Test",
    "contact": [{
      "telecom": [{
        "system": "url",
...
- This SearchParameter includes an xpath element for completeness, but the randomenterprisesolutions FHIR Server does not use the XPath during extraction; it only uses the expression (FHIRPath).
...
Each time a resource is created, updated or reindexed, the randomenterprisesolutions FHIR Server evaluates the FHIRPath expression applicable to the resource type and indexes the values of the matching elements, making these available via a search where the query parameter name matches the `code` element on the `SearchParameter` definition.
...
### 1.3 Filtering
The randomenterprisesolutions FHIR Server supports the filtering of search parameters through `fhir-server-config.json`. The default behavior of the randomenterprisesolutions FHIR Server is to consider all built-in and tenant-specific search parameters when storing resources or processing search requests, but you can configure inclusion filters to restrict the randomenterprisesolutions FHIR Server's view to specific search parameters on a given resource type.

Why would you want to filter built-in search parameters? The answer lies in how search parameters are used by the randomenterprisesolutions FHIR Server. When the FHIR server processes a _create_ or _update_ operation, it stores the resource contents in the datastore, along with search index information that is used by the randomenterprisesolutions FHIR Server when performing search operations. The search index information stored for a particular resource instance is driven by the search parameters defined for that resource type. Therefore if you are storing a resource whose type has a lot of built-in search parameters defined for it (e.g. `Patient`), then you could potentially be storing a lot of search index information for each resource.
...
The search parameter filtering feature is supported through a set of inclusion rules specified via the `fhirServer/resources` property group in `fhir-server-config.json`. The search parameter inclusion rules allow you to define a set of search parameters per resource type that should be included in the randomenterprisesolutions FHIR Server's view of search parameters when storing resources and performing search operations. The following snippet shows the general form for specifying inclusion rules:
...
Note: `_content` and `_query` are not yet supported by the randomenterprisesolutions FHIR Server.

The filter `"*": "*"` is not necessary to include these Resource-level parameters.

#### 1.2.1 Handling unexpected search parameters
The randomenterprisesolutions FHIR Server supports configurable handling of unknown or unsupported search parameters as defined at https://randomenterprisesolutions.github.io/fhir-server/Conformance#http-headers.
...
##  2 Re-index
Reindexing is implemented as a custom operation that tells the randomenterprisesolutions FHIR Server to read a set of resources and replace the existing search parameters with those newly extracted from the resource body.
...
By specifying the index IDs on the `$reindex` operation, the randomenterprisesolutions FHIR Server avoids the database overhead of choosing the next resource to reindex and updating the reindex_tstamp. Though it requires the client side to track the reindex progress, it should allow for an overall faster reindex.

### 2.3 fhir-bucket
To aid in the re-indexing process, the randomenterprisesolutions FHIR Server team has expanded the fhir-bucket resource-loading tool to support driving the reindex, with the option of using either the server-side-driven or client-side-driven approach. The fhir-bucket tool uses a thread-pool to make concurrent POST requests to the randomenterprisesolutions FHIR Server `$retrieve-index` and `$reindex` custom operations.

For more information on driving the reindex operation from fhir-bucket, see https://github.com/randomenterprisesolutions/FHIR/tree/main/fhir-bucket#driving-the-reindex-custom-operation.

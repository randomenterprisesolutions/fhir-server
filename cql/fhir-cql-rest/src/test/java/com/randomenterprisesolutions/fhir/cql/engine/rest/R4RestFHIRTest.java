 ...
 */
package com.randomenterprisesolutions.fhir.cql.engine.rest;
...
public abstract class R4RestFHIRTest {
    ...
    public void mockNotFound(String resource) throws Exception {
        OperationOutcome outcome =
                OperationOutcome.builder().text(Narrative.builder().div(Xhtml.from("<div>generated</div>")).status(NarrativeStatus.builder().value("generated").build()).build()).issue(OperationOutcome.Issue.builder().severity(IssueSeverity.ERROR).code(IssueType.PROCESSING).diagnostics(com.randomenterprisesolutions.fhir.model.type.String.of(resource)).build()).build();
